...
from src.backend.authorization.yandex_client import build_auth_url, exchange_code_for_token, get_user_info
from src.backend.authorization.jwt_utils import create_access_token, decode_access_token

from src.backend.authorization.database.orm_db import OrmDatabaseManager, get_db_manager
from src.backend.authorization.models import User, UserRead, UserUpdate, UserFilter
...
async def callback(
    ...
    await db_manager.save_refresh_token(refresh_token, email, time.time() + settings.REFRESH_TOKEN_EXPIRE_SEC)
...
async def refresh(
    ...
    token_data = await db_manager.get_refresh_token(refresh_token)
    if not token_data or time.time() > token_data["expires"]:
        raise HTTPException(status_code=401, detail="Invalid or expired refresh token")

    new_access_token = create_access_token(token_data["email"])
